...
> **2026-05-05 fold-in note:** Pass 3 sender-ban instant-kill is now scheduled as part of the event-driven architecture rollout — see [event-driven-architecture.md](event-driven-architecture.md) § "Sender-ban code detection (folds in Plan D Pass 3)". A `notify_sender_ban_detected` trigger fires on `response_messages` INSERTs where the bounce body matches the sender-ban code regex. The handler keeps existing alert behavior and adds an instant-kill code path gated by `SENDER_BAN_INSTANT_KILL` env flag for safe rollout. No separate Pass 3 implementation needed.
