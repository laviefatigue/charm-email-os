...
7. Add `domains.qualifies_for_cancellation_at` + `_reason` columns (DECISION 6 schema). Empty initially.
8. Wire kill-trigger evaluator to write the verdict columns (DECISION 6 implementation). Backfill any historical verdicts that can be reconstructed; leave NULL otherwise.
9. Ship change-tracking trigger + `hypertide_status_events` table (DECISION 4). Verdict reader reads `domains.qualifies_for_cancellation_at` per DECISION 6's logic.
10. Cleanup: drop `workspaces.manages_via_hypertide` and `clients.workspace_id` after all reads migrated. **Keep `domains.is_legacy`** per Concern C; just add a comment redefining its semantic.
