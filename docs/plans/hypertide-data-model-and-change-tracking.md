...
created: 2026-05-18
updated: 2026-05-18
status: shipped (steps 3-5, 7-9 in prod; 6 + 10 deferred as low-value — see step 6 notes)
...
6. ~~Migrate operational reads to `v_operational_*` views~~ — **deferred 2026-05-18**, urgency recalibrated. Inventory shows ~530 call sites across 80+ files. Pre-implementation audit (2026-05-18) found NO actual leakage in current production state: F&F clients have zero workspaces (chs-only registrations from hypertide-worker first-sync, which never creates workspaces), and the 4 inactive-client workspaces (Estrada, EventPanda, Neon, Test Workspace) already have `is_active=FALSE`, `automation_enabled=FALSE`, `manages_via_hypertide=FALSE` — every operational sync_module filters on `is_active=TRUE`, so they're structurally excluded. Step 6 was framed as Concern A's "biggest risk item" but the existing `is_active`-based filters cover the current data shape. Re-open this step if: (a) operator manually assigns a workspace to a F&F client (no automation does this today), or (b) we want to drop the manages_via_hypertide column per step 10. Until then, the v_operational_* views remain available as opt-in for new operational code.
...
10. Cleanup: drop `workspaces.manages_via_hypertide` and `clients.workspace_id` after all reads migrated. **Gated on step 6** (now deferred). **Keep `domains.is_legacy`** per Concern C; just add a comment redefining its semantic. Re-prioritize when step 6 is re-opened.
