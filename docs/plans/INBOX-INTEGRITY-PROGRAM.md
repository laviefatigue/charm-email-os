...
| [eod-campaign-reapply.md](eod-campaign-reapply.md) | ~600 | **v2 daemon LIVE IN APPLY-MODE — FLEET ROLLOUT COMPLETE** | v1 CLI + v2 event-driven daemon both shipped. Migration 111 applied. Daemon deployed as Coolify `eod-reapply-daemon`: enqueuer (per-tz `trigger_at`) + worker (claim → orchestrate → finalize) + startup crash-recovery scan. Apply-mode cut over 2026-05-14. First real apply-run validated on SPUI campaign 101 (EB-API proof `attached == live`); verify-loop fetch-retry bug found + fixed (`89e58a0`). **2026-05-14: all 10 active workspaces except Sammy enabled — 30 jobs queued across 7 workspaces with active campaigns, each at its own EOD/tz.** 249 tests. | PR 3 validation audit optional/deferred. Otherwise complete — steady-state operation. |
