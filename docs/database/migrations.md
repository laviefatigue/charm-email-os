...
| `100_rename_pool_deployed_to_live.sql` | **ADR-008 stepping-stone:** Renames `sender_accounts.inventory_pool_status` value `'deployed'` → `'live'`. Aligns with the EB tag name (`live`) and `domains.pool_status='live'` so the same vocabulary is used everywhere. ~1,871 active rows updated. Companion: production code, tests, scripts, and active docs updated to use `'live'` consistently. Same-week stepping stone toward the eventual ADR-008 collapse of `inventory_lifecycle_status` + `inventory_pool_status` into a single `inbox_status` column. |
