...
def _print_audit_result(r: AuditResult, applied: bool) -> None:
    ...
    click.echo("")
    click.echo("  --- subscription-level (chs sync) ---")
    click.echo(f"  Unique subs in this pass:        {r.chs_sync.subs_seen}")
    click.echo(f"   ├─ already bound (touched):     {r.chs_sync.already_bound}")
    click.echo(f"   ├─ first-sight → new client:    {r.chs_sync.new_clients_client_status}")
    ...
    click.echo(f"  HT order without DB domain row:  {r.ht_no_db_row}  (F&F + Instantly-extraction-pending)")
