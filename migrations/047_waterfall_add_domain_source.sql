-- ============================================
-- WATERFALL VIEW V3 - Add domain_source for legacy detection
-- ============================================
-- Adds domain_source column to support legacy domain identification in UI

BEGIN;

-- Drop the existing view first (required to change column order)
DROP VIEW IF EXISTS v_infrastructure_waterfall;

-- Recreate the view with domain_source
...
SELECT
    ...
    -- Domain source (legacy, generated, purchased)
    ...
    -- Status (Column 6) - Live/Dead inbox counts
...
COMMENT ON VIEW v_infrastructure_waterfall IS
  'Waterfall view for infrastructure provisioning SPA. Columns: Domain, Pricing, DNS, Provider, HyperTide, Status. Includes domain_source for legacy detection.';

COMMIT;

SELECT 'Migration 047_waterfall_add_domain_source complete' AS status;
